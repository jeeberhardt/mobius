...
class Problem(ElementwiseProblem):
    ...
    def _evaluate(self, x, out, *args, **kwargs):
        ...
        """
        # Check if polymer has been seen before
        if x[0] in self._polymer_cache:
            scores = self._polymer_cache[x[0]]
        else:
            ...
            if self._pre_evaluation and x[0] in self._prior_data:
                scores = self._prior_data[x[0]]
                scores = scores.reshape(scores.shape[0], -1)
            elif not self._pre_evaluation:
                scores = []

                # Evaluate unseen polymer with acquisition scores from surrogate models
                for acq_fun in self._acq_funs:
                    prediction = acq_fun.forward(x)
                    acq_score = acq_fun.scaling_factor * prediction.acq
                    scores.append(acq_score)

                scores = np.asarray(scores, dtype=float)

                # Record acquisition score for found polymer in cache
                self._polymer_cache[x[0]] = scores
            else:
                msg = f'Polymers {x[0]} not found in the input experimental data. '
