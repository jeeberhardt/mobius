...
def constrained_embed_multiple_confs(query_mol, core_mol, num_confs=10, cluster=True, distance_cutoff=1.0, random_seed=0xf00d, mcs_timeout=1):
    ...
    cids = AllChem.EmbedMultipleConfs(query_mol, num_confs, ps)
    cids = list(cids)
    ...
    for cid in cids:
        ...
        # Do first minimization with UFF to superpose all the query atoms on the top of the core atoms
        ff = AllChem.UFFGetMoleculeForceField(query_mol, confId=cid)
        ...
        # Do a last minimization with MMFF to relax the whole query molecule without
        # deviating too far from the initial placement
        mp = AllChem.MMFFGetMoleculeProperties(query_mol)
        ff = AllChem.MMFFGetMoleculeForceField(query_mol, mp, confId=cid)

        for atom_pair in cmap:
            ff.MMFFAddPositionConstraint(atom_pair[0], max_displacement, force_constant)
...
class VinaScorer:
    ...
    def dock(self, input_molecule, template=None, num_confs=10, cluster=True, adjust_protonation=True, pH=7.4):
        ...
        if adjust_protonation:
            input_molecule = Chem.RemoveHs(input_molecule)
            input_molecule = self._protonator(input_molecule, pH, pH)[0]
            input_molecule = Chem.AddHs(input_molecule, addCoords=True)
